...
const SupabaseTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const { data: allData, error } = await supabase.from("xpense").select("*");
  ...
  const sumElectricity = rows
    .filter((row) => row.values.category === "electricity")
    .reduce((total, row) => total + parseFloat(row.values.amount), 0)
    .toFixed(2);
  ...
  return (
    <>
      ...
      <div className="flex justify-center mt-4">
        ...
        <p className="mx-4">Total amount of electricity: {sumElectricity}</p>
      </div>
    </>
  );
};
